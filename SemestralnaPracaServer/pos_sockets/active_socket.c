...
void active_socket_write_data_aspon_toto_funguje(struct active_socket* self, const char* message) {
    pthread_mutex_lock(&self->mutex_writing);
    write(self->socket_descriptor, message, strlen(message));
    write(self->socket_descriptor, SOCKET_TERMINATE_CHAR, sizeof(SOCKET_TERMINATE_CHAR));
    pthread_mutex_unlock(&self->mutex_writing);
}
...
////spravene
//void active_socket_send_message(struct active_socket* self, const char* message) {
//    pthread_mutex_lock(&self->mutex_writing);
//
//    CHAR_BUFFER message_buffer;
//    char_buffer_init_copy(&message_buffer, (const struct char_buffer *) message);
//    active_socket_write_data(self, &message_buffer);
